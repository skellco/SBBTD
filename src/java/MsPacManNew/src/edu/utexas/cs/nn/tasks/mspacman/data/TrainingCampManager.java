...
public class TrainingCampManager {
    ...
    private void prepareCamps(int gen) {
        if (gen >= 0) {
            assessPreviousCamps();
            int campLimit = Math.min(
                    (int) Math.floor(CommonConstants.campPercentOfTrials * CommonConstants.trials),
            ...
            if (campLimit > 0) {
                ...
                int numDeathTrials = (int) Math.ceil(Parameters.parameters.doubleParameter("percentDeathVsPPCamps") * campLimit);
    ...
    public void preEval() {
        ...
        if (recentStates != null || viewFinalCamps) {
            ...
            if (!finalEval) {
                ...
                for(int i = 0; i < keeperCamps.size(); i++) {
                    FileUtilities.simpleFileWrite(keeperPath + "/KeeperCamp-" + keeperCamps.size() + ".txt", keeperCamps.get(i));
                }
            }
        }
    }
    ...
    public void postEval(GameFacade game, int campNum, int startingLevel) {
        // Means pacman died at end of eval (not time limit or cleared all levels)
        int remainingLives = game.getPacmanNumberOfLivesRemaining();
        if (remainingLives == 0) {
            NNPacManController.timesDied++;
            if (recentStates != null
                    && recentStates.memoryFull()
                ...
                FileUtilities.simpleFileWrite(genPath + "/PreDeathState-" + gen + "-" + (saveNumber++) + ".txt", shortlyBeforeDeath.getGameState());
            }
        }
    ...
    public static void ghostEatingCamp(int time) {
        ...
        if (TrainingCampManager.recentStates != null
                && TrainingCampManager.recentStates.memoryFull()
            ...
            FileUtilities.simpleFileWrite(genPath + "/PrePowerPillState-" + gen + "-" + time + ".txt", shortlyBeforeEatingPowerPill.getGameState());
        }
    }
    ...
    private void assessPreviousCamps() {
        ...
        for (int i = campPerformance.size() - 1; i >= 0; i--) {
            ...
            System.out.println("Camp " + i + ": " + performance + "/" + maxEvals + "(" + ((100.0 * performance) / maxEvals) + "%)");
            if (performance == 0) {
