...
public abstract class FitnessBasedIncrementalEvolutionProcess<T> {
    ...
    private FitnessBasedIncrementalEvolutionProcessLog stateHistory; // needed for resumes
    ...
    public void updateRecencyWeightedAverage(ArrayList<Score<T>> parentScores, int generation) {
        ...
        if (recencyWeightedAverage > goal && performance > goal) { // second check unnecessary?
            increaseDifficulty();
     ...
     *
     * @param s Score instance containing all scores associated with organism's
     * performance.
     * @return
     */
