...
public class DeathLocationsLog extends MONELog {
     ...
     *
     * @param deaths (location, death count) hash
     * @return
     */
    ...
    public void heatMap(GameFacade g) {
        ...
        if(keys != null) {
            for (Integer intensity : keys) {
                ...
                g.addPoints(new Color(Math.min(255, intensity), Math.min(255, Math.max(intensity - 255, 0)), Math.min(255, Math.max(intensity - (2 * 255), 0))), locations);
            }
        }
    }
}
