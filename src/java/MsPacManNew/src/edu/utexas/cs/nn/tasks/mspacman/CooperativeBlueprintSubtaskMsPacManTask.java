 ...
 * @author Jacob Schrum
 */
public class CooperativeBlueprintSubtaskMsPacManTask<T extends Network> extends CooperativeSubtaskSelectorMsPacManTask<T> implements BlueprintTask {
     ...
     *
     * @param populations different populations of genotypes, each the same
     * size. Populations are: 0=Selectors,1=Ghosts,2=Pills,3=Blueprints.
    ...
    public ArrayList<ArrayList<Score>> evaluateAllPopulations(ArrayList<ArrayList<Genotype>> populations) {
        ...
        // For each blueprint, fill slots in each column of team order to define team
            ...
            for (int p = 0; p < pops; p++) {
                ...
                if (p == blueprintPopIndex) {
                    ...
                    if (index == -1) {
                        ...
                        //System.out.println("Will look for network " + id + " of pop " + p + " in parent population");
                    } else {
                        fullParent = false;
                    }
     ...
     * making the team.
     *
     * @param populations all populations, with blueprint population last
     * @param joinOrder order to eval teams
     * @param index index in join order groups
    ...
    protected Genotype[] getTeam(ArrayList<ArrayList<Genotype>> populations, List<ArrayList<Integer>> joinOrder, int index) {
        ...
        for (int p = 0; p < pops; p++) {
            ...
            if (popSlot == -1) { // Network is in a parent population
                ...
                //System.out.println("Net " + team[p].getId() + " found in parent pop as expected");
     ...
     *
     * @param rawSums Array that builds up to hold sum of scores across all
     * evals
     * @param teamOrder eval order for teams
     * @param order index in team order groups
     * @param scores scores for the networks from evaluated team
     */
    @Override
    protected void addScores(Score[][] rawSums, List<ArrayList<Integer>> teamOrder, int order, ArrayList<Score> scores) {
        int blueprintPop = rawSums.length - 1;
        for (int p = 0; p < scores.size(); p++) {
            if (p == SELECTOR_POP_INDEX) {
                int orderIndex = teamOrder.get(blueprintPop).get(order); // index of blueprint
            ...
            if (orderIndex == -1) { // Add to a parent pop score
                ...
                //System.out.println("Updated parent score " + parentScore.individual.getId() + " in pop " + p);
            } else {
    ...
    @Override
    protected ArrayList<ArrayList<Score>> wrapUpScores(Score[][] rawSums, ArrayList<ArrayList<Genotype>> populations, List<ArrayList<Integer>> teamOrder) {
        /**
     ...
     *
     * @param bp child blueprint
     * @param joinOrder join order for child populations
     * @param index join order column index
     * @param popIndex subpop index
