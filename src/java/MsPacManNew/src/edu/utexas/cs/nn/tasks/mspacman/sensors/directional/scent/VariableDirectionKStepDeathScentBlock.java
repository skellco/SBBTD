...
//    public static int[] sumDeathScents = new int[Constants.NUM_MAZES];
    ...
    public static void updateScentMaps(){
        if(scentMaps != null) {
            scentMaps = ExecutorNew.deaths.deathCount();
...
//                    sumDeathScents[i] = StatisticsUtilities.sum(deaths);
                    Collections.sort(deathCounts, Collections.reverseOrder()); // Descending order
    ...
    public double getValue(GameFacade gf) {
        ...
        //double maxCount = maxDeathScents[gf.getMazeIndex()] * stepCount; // Maximum deaths at each location checked
        //double maxCount = sumDeathScents[gf.getMazeIndex()];
        double maxCount = topSumDeathScents[gf.getMazeIndex()];
        //System.out.println(dir + ":death scent:" + count + ":max:"+maxCount + ":=" + (count/maxCount));
        return maxCount == 0 ? 0 : count / maxCount; // Normalized
    }

    public static int countScentAlongPath(GameFacade gf, ArrayList<Integer> visited, HashMap<Integer,Integer> previousDeathsAtLocations, int sourceLocation, int currentLocation, int remainingSteps, boolean max) {
        ...
        if (remainingSteps == 0) {
            ...
            for (int i = 0; i < neighbors.length; i++) {
                if (neighbors[i] != -1) {
                    int count = countScentAlongPath(gf, visited, previousDeathsAtLocations, currentLocation, neighbors[i], remainingSteps - 1, max);
            ...
            int targetsToCount = alreadyCounted ? 0 : (previousDeathsAtLocations.containsKey(currentLocation) ? previousDeathsAtLocations.get(currentLocation) : 0);
