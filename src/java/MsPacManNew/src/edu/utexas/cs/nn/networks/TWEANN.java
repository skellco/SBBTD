...
public class TWEANN implements Network {
    ...
    public static boolean preferenceNeuron() {
        ...
        return (!multitaskCombiningCrossover || startingModes > 1 || mmpRate > 0 || mmrRate > 0 || mmdRate > 0 || fullMMRate > 0);
    }
    ...
    public TWEANN(TWEANNGenotype g) {
        ...
        assert (numModes == 1 || numOut == (neuronsPerMode + (multitask || CommonConstants.ensembleModeMutation ? 0 : 1)) * numModes) :
                "multitask:" + multitask + "\n"
...
//        }
        ...
        if (CommonConstants.ensembleModeMutation) {
            ...
            if (canDraw && preferenceNeuronPanel != null && numModes > 1 && !multitask) {
        ...
        if (canDraw) {
            ...
            if (inputPanel != null) {
                ...
                refreshActivation(inputPanel, inputs, outputs, preferences, multitask, preferenceFatigue);
            }
        }
    ...
    public double[] modeOutput(int mode) {
        int selectedModeStart = outputStart + (mode * (neuronsPerMode + (multitask ? 0 : 1)));
        double[] outputs = new double[neuronsPerMode];
    ...
    public void flush() {
        ...
        if (canDraw && preferenceNeuronPanel != null && !multitask && numModes > 1) {
        ...
        this.preferenceFatigue = new double[numModes];
        if (inputPanel != null) {
            refreshActivation(inputPanel, new double[numIn], new double[neuronsPerMode], new double[numModes], multitask, new double[numModes]);
        }
    }
...
//        }
    ...
    private static void refreshActivation(DrawingPanel inputPanel, double[] inputs, double[] outputs, double[] preferences, boolean multitask, double[] preferenceFatigue) {
        ...
        for (int j = 0; j < preferences.length; j++, i++) {
            ...
            g.setColor(CombinatoricUtilities.colorFromInt(j));
            g.fillRect(x, y - (Plot.OFFSET / 2), w, Plot.OFFSET / 2);
            // show fatigue
            g.setColor(Color.RED);
        ...
        for (int l = 0; l < layers.size(); l++) {
            ...
            for (int n = 0; n < layer.size(); n++) {
                ...
                if (display.ntype == Node.NTYPE_OUTPUT) {
                    ...
                    if (n / (neuronsPerMode + (multitask ? 0.0 : 1.0)) == chosenMode) {
                        g.setColor(CombinatoricUtilities.colorFromInt(chosenMode));
                        g.fillRect(x, 0, 2 * NODE_DIM, 2 * NODE_DIM); // erase mode indicator
                    }
                    if (multitask || CommonConstants.ensembleModeMutation || n % (neuronsPerMode + 1) == neuronsPerMode) {
            ...
            for (int n = 0; n < layer.size(); n++) {
                ...
                for (Link disLink : display.outputs) {
                    ...
                    if (target == null) {
                        ...
                        System.out.println("multitask:" + this.multitask);
