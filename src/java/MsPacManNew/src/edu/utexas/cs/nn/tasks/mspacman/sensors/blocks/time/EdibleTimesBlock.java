...
public class EdibleTimesBlock extends MsPacManSensorBlock {
    ...
    public int incorporateSensors(double[] inputs, int in, GameFacade gf, int lastDirection) {
        ...
        //System.out.println(Arrays.toString(mask) + Arrays.toString(times) + ":" + Constants.EDIBLE_TIME);
        for (int i = times.length - 1; i >= 0; i--) { 
            if (mask[i]) { 
                //inputs[in++] = times[i] == 0 ? absence : times[i] / ((Constants.EDIBLE_TIME - DANGEROUS_TIME) * 1.0);
