...
public class TWEANNGenotype implements Genotype<TWEANN> {
    ...
    public TWEANNGenotype(TWEANNGenotype copy) {
        this(copy.nodes, copy.links, copy.neuronsPerMode, copy.multitask, copy.archetypeIndex);
    }
     ...
     */
    public TWEANNGenotype(ArrayList<NodeGene> nodes, ArrayList<LinkGene> links, int neuronsPerMode, boolean multitask, int archetypeIndex) {
    ...
    private int numModes() {
        return (int) Math.max(1, numOut / (neuronsPerMode + (multitask || CommonConstants.ensembleModeMutation ? 0 : 1)));
    }
...
//    }
     ...
     *
     * @param linkIndex index in "links". CAnnot be frozen.
     * @param w new weight
     */
