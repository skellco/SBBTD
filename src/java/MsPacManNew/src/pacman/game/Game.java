...
//        }
    ...
    /**
     * Initiates a new game from an SBB Point
     *
     * @param seed Seed used for the pseudo-random numbers
     * @param point SBB point as double[]
     */
    public Game(long seed, float[] point) {
        ArrayList<Integer> occupied = new ArrayList<Integer>();
        rnd = new ResumableRandom(seed);
        int index = 0;
        //game over after current level
        Parameters.parameters.setInteger("pacmanMaxLevel",(int)Math.floor(point[index]*NUM_MAZES));
        //level
        _init((int)Math.floor(point[index++]*NUM_MAZES));
        int[] junctions = getJunctionIndices();
        //Pac-Man initial location
        int pacmanNodeIndex = junctions[(int)Math.floor(point[index++]*junctions.length)];
        occupied.add(pacmanNodeIndex);
        pacman = new PacMan(pacmanNodeIndex, MOVE.UP, 1, false);
        //ghost initial locations and edible times
        ghosts = new EnumMap<GHOST, Ghost>(GHOST.class);   
        int[] pillIndices = getPillIndices();
        for (GHOST ghostType : GHOST.values()) {
        	int i = (int)Math.floor(point[index++]*pillIndices.length);
        	int nodeIndex = pillIndices[i];
        	while(occupied.contains(nodeIndex) || Math.abs(pacmanNodeIndex - nodeIndex) < 5) nodeIndex = pillIndices[++i%pillIndices.length]; 	
        	occupied.add(nodeIndex);
            ghosts.put(ghostType, new Ghost(ghostType, 
            		nodeIndex, 
            		0,
                    0, 
                    MOVE.NEUTRAL));
        }
        //pill locations with sparseness parameter
        _setPills(currentMaze,point[index++]);
        //power pills present?
        if (point[index] < 0.5)
        	powerPills.clear();
    }
    ...
    public boolean getSimultaneousLairExit() {
        return simultaneousLairExit;
    }
    ...
    public boolean getRandomLairExit() {
        return randomLairExit;
    }
    ...
    public boolean getExitLairEdible() {
        return exitLairEdible;
    }
    ...
    public boolean getPlayWithoutPowerPills() {
        return noPowerPills;
    }
    ...
    public boolean getEndAfterPowerPillsEaten() {
        return endAfterPowerPillsEaten;
    }
    ...
    public boolean getEndAfterGhostEatingChances() {
        return endAfterGhostEatingChances;
    }
    ...
    public boolean getPlayWithoutPill() {
        return playWithoutPillsUntilGhostsThreaten;
    }
    ...
    public boolean getEndOnlyOnTimeLimit() {
        return endOnlyOnTimeLimit;
    }
    ...
    /**
     * _set pills, specifying the proportion of total pills to use
     *
     * @param maze the maze
     * @param proportionPillsToRemove proportion of pills to remove
     */
    private void _setPills(Maze maze, float proportionPillsToRemove) {
        pills = new BitSet(currentMaze.pillIndices.length);
        pills.set(0, currentMaze.pillIndices.length);
        //System.out.println("Maze " + this.mazeIndex + ": " + currentMaze.pillIndices.length);
        if (playWithoutPillsUntilGhostsThreaten) {
        ...
        // Randomly eat/remove a portion of the pills before the level starts.
        // Makes the task easier along a smooth gradient.
        if (proportionPillsToRemove > 0) {
            int numPillsToEat = (int) Math.floor(proportionPillsToRemove * currentMaze.pillIndices.length);
            int[] pillNodesToEat = RandomNumbers.randomXofY(numPillsToEat, currentMaze.pillIndices.length);
            for (int node : pillNodesToEat) {
                pills.clear(node);
            }
        ...
        for (Ghost ghost : ghosts.values()) {
            ...
            if (distance <= EAT_DISTANCE && distance != -1) {
                ...
                } else //ghost eats pac-man
                {
                    ...
                    pacman.numberOfLivesRemaining--;
                    pacmanWasEaten = true;
                    if (!CommonConstants.watch && ExecutorNew.deaths != null) {
                        ExecutorNew.deaths.log(this.mazeIndex + ":" + this.getPacmanCurrentNodeIndex());
                    }
