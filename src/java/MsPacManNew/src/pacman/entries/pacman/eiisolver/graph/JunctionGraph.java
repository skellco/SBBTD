...
public class JunctionGraph {
     ...
     *
     * @param index1 index of source node(normal index, not junction index) of
     * from node
    ...
    private void calcGhostDist() {
        ...
        for (BigEdge edge : edges) {
            ...
            Log.println(edge.endpoints[0] + " " + edge.firstMoveToOtherEnd[0].opposite() + " - "
                    ...
                    + ghostDist[edge.firstMoveToOtherEnd[0].opposite().ordinal()][edge.endpoints[0].junctionIndex][edge.firstMoveToOtherEnd[1].ordinal()][edge.endpoints[1].junctionIndex]);
            Log.println(edge.endpoints[1] + " - " + edge.endpoints[0] + ": "
                    + ghostDist[edge.firstMoveToOtherEnd[1].opposite().ordinal()][edge.endpoints[1].junctionIndex][edge.firstMoveToOtherEnd[0].ordinal()][edge.endpoints[0].junctionIndex]);
            ...
            for (int e = 0; e < n.nrNeighbours; ++e) {
                ...
                for (int j = 0; j < junctionNodes.length; ++j) {
                    for (int e2 = 0; e2 < junctionNodes[j].nrNeighbours; ++e2) {
                        ...
                        ghostDistToJunc[MOVE.NEUTRAL.ordinal()][i][moveFromIndex2.ordinal()][j] = getGhostDistToJunctionInternal(i, MOVE.NEUTRAL, junctionNodes[j].index, moveFromIndex2);
                        ghostDistToJunc[lastMoveMade.ordinal()][i][MOVE.NEUTRAL.ordinal()][j] = getGhostDistToJunctionInternal(i, lastMoveMade, junctionNodes[j].index, MOVE.NEUTRAL);
                    }
                }
            }
        }
