...
keepaway_dir=~/Shootgoal/keepaway-0.4
...
ka_width=30                      # Y-axis size of playing region
ka_length=$[ka_width*2]              # X-axis size of playing region
...
trainer_monitor=1                # should I launch the trainer monitor on startup?
save_trainer_log=0               # should I save trainer's world state queue to a file?
trainer_dir=~/Shootgoal/Trainer  # directory containing trainer files
trainer=Trainer                  # name of trainer binary/class

bounce=0                         # should I turn on "bounce" mode?
elasticity=0.9                   # elasticity of wall collisions
torus=0                          # map region onto torus topology
move_speed=0.25                  # speed at which region moves down field
...
server_opts="$s::port=$port $s::coach_port=$coach_port $s::olcoach_port=$olcoach_port $s::half_time=-1 $s::forbid_kick_off_offside=0 $s::use_offside=0 $s::stamina_inc_max=3500 $s::synch_mode=$synch_mode $keepaway_opts $log_opts $vision_opts $s::coach=$use_trainer"
...
if (( $save_trainer_log && $use_trainer )); then
  trainer_log_opts="$trainer_log_opts -queue_file $log_dir/$proc_name.que"
fi

echo Starting Server....
#echo rcssserver $server_opts
~/ShootGoal/bin/rcssserver $server_opts &
server_pid=$!
...
if (( $use_trainer )); then
  pushd $trainer_dir
  trcmd_line="java $trainer -port $coach_port -monitor $trainer_monitor -keepers $num_keepers -takers $num_takers -width $ka_width -length $ka_length -bounce $bounce -elasticity $elasticity -torus $torus -move_speed $move_speed $trainer_log_opts"
